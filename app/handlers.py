...
# --- ИМПОРТ ФУНКЦИИ ДЛЯ ОТПРАВКИ УВЕДОМЛЕНИЙ ВО ВТОРОЙ БОТ ---
from bot import send_admin_notification

load_dotenv()
router = Router()

# Твой кошелек для получения GRAM от клиентов
...
async def price(message: Message, state: FSMContext):
    await state.set_state(None)
    # Берем настройки прямо из переменных Railway
    ...
    await message.reply(f"Цена за одну звезду 🌟 составляет <b>{price_rub} рубля</b> (или примерно {price_usdt} USDT)."
                        "\n\nПри оформлении заказа сумма пересчитывается в GRAM по актуальному курсу. 📈",
                        parse_mode="HTML")
...
async def how_it_works(message: Message, state: FSMContext):
    ...
    await message.reply(""" 
            ...
            \n3.  <b>Оплата.</b> Переводишь нужную сумму в GRAM на наш кошелек. 💰
...
async def choose_status(callback: CallbackQuery, state: FSMContext):
    ...
    if data["purchase_method"] == "USDT":
        await callback.message.answer(
            f"💳 <b>Для оплаты переведите {total_gram} GRAM на кошелек:</b>\n\n"
        ...
        # ОТПРАВКА УВЕДОМЛЕНИЯ АДМИНУ О НОВОМ ЗАКАЗЕ (ВО ВТОРОЙ БОТ)
        await send_admin_notification(
            f"💎 **НОВЫЙ ЗАКАЗ (Ожидает оплаты)!**\n"
            f"👤 Клиент: @{data['username']}\n"
            f"⭐ Количество: {data['amount']} ⭐\n"
            f"💰 Сумма к оплате: {total_gram} GRAM\n"
            f"🆔 ID заказа: {new_order.id}\n"
            f"📦 Получатель: @{data['username']}\n"
            f"💳 Кошелек: {MY_WALLET[:10]}...\n"
            f"🕐 Ожидание: пользователь должен оплатить."
        )
...
async def payment_status(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    # ОТПРАВКА УВЕДОМЛЕНИЯ АДМИНУ О ТОМ, ЧТО КЛИЕНТ НАЖАЛ "ОПЛАТИЛ" (ВО ВТОРОЙ БОТ)
    await send_admin_notification(
        f"⚠️ **КЛИЕНТ НАЖАЛ «ОПЛАТИЛ»!**\n"
        f"🆔 Заказ: {callback.data}\n"
        f"👤 Админ, проверьте поступление GRAM на кошелек!\n"
        f"⏳ После проверки нажмите кнопку подтверждения."
    )
...
async def confirm_payment(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    for order in order_data:
        ...
        # --- ПОЛНОСТЬЮ УБРАНА АВТОМАТИЧЕСКАЯ ПОКУПКА ЧЕРЕЗ API ---
        # Теперь бот просто отправляет админу инструкцию для ручной покупки через @Fragment
        ...
        # Отправляем уведомление админу (во второй бот)
        await send_admin_notification(
            f"📢 **НУЖНО КУПИТЬ ЗВЁЗДЫ!**\n"
            f"🆔 Заказ: #{transaction_id}\n"
            f"👤 Получатель: @{order.username}\n"
            f"⭐ Количество: {order.amount} ⭐\n\n"
            f"➡️ Откройте @Fragment -> Stars -> Buy for others.\n"
            f"➡️ Введите {order.amount} и получателя @{order.username}.\n"
            f"➡️ Подтвердите оплату в кошельке."
        )
        # --- КОНЕЦ ИЗМЕНЕНИЙ ---


@router.callback_query(F.data == "payment_cancel")
async def payment_cancel(callback: CallbackQuery, state: FSMContext, bot: Bot):
    await state.clear()

...
async def confirm_payment(callback: CallbackQuery, state: FSMContext, bot: Bot):
    # Проверяем, что нажал именно админ (по ID из переменных)
    ...
    for order in order_data:
        ...
        try:
            if stars_buy["success"]:
                ...
                # ОТПРАВКА УВЕДОМЛЕНИЯ АДМИНУ ОБ УСПЕШНОМ ЗАЧИСЛЕНИИ ЗВЕЗД (ВО ВТОРОЙ БОТ)
                await send_admin_notification(
            ...
            # ОТПРАВКА УВЕДОМЛЕНИЯ АДМИНУ ОБ ОШИБКЕ (ВО ВТОРОЙ БОТ)
            await send_admin_notification(
                f"❌ **ОШИБКА В ЗАКАЗЕ!**\n"
                f"🆔 Заказ: #{transaction_id}\n"

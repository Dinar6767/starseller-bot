...
# --- ФУНКЦИЯ ОТПРАВКИ УВЕДОМЛЕНИЙ ВО ВТОРОЙ БОТ (ПРЯМО ЗДЕСЬ) ---
async def send_admin_notification(text: str):
    """Отправляет уведомление администратору через второго бота"""
    try:
        admin_token = os.getenv("ADMIN_BOT_TOKEN")
        admin_id = int(os.getenv("ADMIN_ID"))
        if not admin_token or not admin_id:
            print("❌ В переменных Railway не указан ADMIN_BOT_TOKEN или ADMIN_ID")
            return

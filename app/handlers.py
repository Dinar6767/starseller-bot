...
async def payment_status(callback: CallbackQuery, state: FSMContext, bot: Bot):
    ...
    # ОТПРАВКА УВЕДОМЛЕНИЯ АДМИНУ О ТОМ, ЧТО КЛИЕНТ НАЖАЛ "ОПЛАТИЛ"
    # Кнопки для подтверждения отправляются прямо в чат админ-бота!
    await send_admin_notification(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="Подтвердить оплату ✅", callback_data="adm_conf:" + callback.data)],
            [InlineKeyboardButton(text="Отмена ❌", callback_data="cancel")]
        ])
    )
...
# ==========================================================
# 👇 ЭТО ФИНАЛЬНАЯ ИСПРАВЛЕННАЯ ФУНКЦИЯ (КОТОРАЯ ТОЧНО РАБОТАЕТ)
# ==========================================================
...
async def confirm_payment(callback: CallbackQuery, state: FSMContext, bot: Bot):
    # 1. МГНОВЕННО снимаем "часики" на кнопке, чтобы она не зависла
    await callback.answer("✅ Обработка...", show_alert=False)
    
    # 2. Жёстко проверяем админа (ваш ID)
    YOUR_ADMIN_ID = 6305430094
    if callback.from_user.id != YOUR_ADMIN_ID:
        await callback.answer("⛔ Только для админа!", show_alert=True)
        return

    # 3. Сразу прячем кнопки (чтобы нельзя было нажать дважды)
    try:
    ...
    for order in order_data:
        await bot.send_message(
            ...
            text=f"🎉 Ваш платеж на {order.amount} ⭐ подтвержден! Звезды зачисляются...",
            reply_markup=kb.main
        )
        
        # 6. Покупаем звёзды через Fragment
        try:
            ...
            if stars_buy["success"]:
                ...
                # 7. Сообщаем админу об успехе
                await send_admin_notification(
                    f"✅ **ЗАКАЗ УСПЕШНО ВЫПОЛНЕН!**\n"
                    f"🆔 Заказ: #{transaction_id}\n"
                    f"👤 Получатель: @{order.username}\n"
                    f"⭐ Звезды: {order.amount} ⭐\n"
                    f"💸 Статус: Зачислены! 🎉"
                )
            ...
            await send_admin_notification(
                f"❌ **ОШИБКА В ЗАКАЗЕ!**\n"
                f"🆔 Заказ: #{transaction_id}\n"
                f"❗ Ошибка при зачислении.\n"
                f"📄 Ошибка: {str(e)}"
            )
    
    # 8. В самом конце отправляем финальное сообщение в чат админа
    await callback.message.edit_text(
        text=f"✅ Платеж #{transaction_id} успешно обработан!",
        reply_markup=None
    )
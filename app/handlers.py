...
# Импорт функции уведомлений из нового bot.py
from bot import send_admin_notification, send_order_notification
...
async def cancel(callback: CallbackQuery, state: FSMContext):
    ...
    await state.set_state(None)
    await callback.message.edit_text(text=f"Отмена ❌", reply_markup=None)
...
async def choose_amount(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(text=f"Вы выбрали {stars_amount} ⭐", reply_markup=None)
    await state.set_state(Purchase.username)
...
async def choose_purchase(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(text=f"Вы выбрали {callback.data}", reply_markup=None)
    await callback.message.answer(f"Ваш заказ: {data['amount']} ⭐\n\nПолучатель: @{data['username']}\n\n"
...
async def choose_status(callback: CallbackQuery, state: FSMContext):
    ...
    if data["purchase_method"] == "USDT":
        ...
        # ОТПРАВКА УВЕДОМЛЕНИЯ АДМИНУ ЧЕРЕЗ ЕДИНУЮ ФУНКЦИЮ
        await send_order_notification(int(data['amount']), data['username'], new_order.id)

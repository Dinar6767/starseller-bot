...
async def confirm_payment(callback: CallbackQuery, state: FSMContext, bot: Bot):
    # ⚠️ ВАШ ID ЖЁСТКО ПРОПИСАН ЗДЕСЬ (чтобы кнопка точно работала)
    YOUR_ADMIN_ID = 6305430094
    
    # Если нажал не ты — выдаём предупреждение и выходим
    if callback.from_user.id != YOUR_ADMIN_ID:
        await callback.answer("⛔ У вас нет прав для этого действия.", show_alert=True)
        return

    await callback.answer("✅ Вы подтвердили платёж! Ожидайте...")
    
    # Достаем ID заказа из callback.data (например, "adm_conf:3" -> 3)
    transaction_id = int(callback.data.split(":")[1])
    
    order_data = await rq.get_chat_id(transaction_id)
    for order in order_data:
        # Отправляем клиенту сообщение об успехе
        ...
        try:
            if stars_buy["success"]:
                ...
                await callback.message.edit_text(f"✅ Платеж #{transaction_id} успешно подтвержден!", reply_markup=None)
                
                # Отправляем уведомление об успехе во второй бот
                await send_admin_notification(
            ...
            await send_admin_notification(
                f"❌ **ОШИБКА В ЗАКАЗЕ!**\n"
                f"🆔 Заказ: #{transaction_id}\n"
                f"❗ Ошибка при зачислении звезд.\n"
                f"📄 Проверьте логи: {str(e)}"
            )